  ...
  return (
    <aside style={{
      ...
      <div style={{ padding: '20px 18px 16px', borderBottom: '0.5px solid var(--bd)' }}>
        ...
        <div style={{ fontSize: 11, color: 'var(--hi)', marginTop: 2 }}>
          {profile?.full_name ?? profile?.email ?? 'Personal finance'}
        </div>
      </div>
      ...
      <div style={{ padding: '12px 18px', borderTop: '0.5px solid var(--bd)' }}>
        <div style={{ fontSize: 11, color: 'var(--hi)', marginBottom: 8, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {profile?.email}
        </div>
        <button
          onClick={signOut}
